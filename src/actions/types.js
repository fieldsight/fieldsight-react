// export const SHOW_DOT_LOADER = "SHOW_DOT_LOADER";

// Submission Detail Actions
export const START_SUBMISSION_LOADER = 'START_SUBMISSION_LOADER';
export const STOP_SUBMISSION_LOADER = 'STOP_SUBMISSION_LOADER';
export const GET_SUBMISSION_DETAIL = 'GET_SUBMISSION_DETAIL';
export const POST_SUBMISSION_DETAIL = 'POST_SUBMISSION_DETAIL';
export const UPDATE_SUBMISSION_DETAIL = 'UPDATE_SUBMISSION_DETAIL';
export const TOGGLE_NULL_SUBMISSIONS_ANSWER =
  'TOGGLE_NULL_SUBMISSIONS_ANSWER';
export const SHOW_SUBMISSION_ERR_MSG = 'SHOW_SUBMISSION_ERR_MSG';

// Site Dashboard Actions
export const GET_SITE_DASHBOARD = 'GET_SITE_DASHBOARD';
export const GET_SITE_METAS = 'GET_SITE_METAS';
export const GET_SITE_SUBMISSIONS = 'GET_SITE_SUBMISSIONS';
export const GET_SITE_DOCUMENTS = 'GET_SITE_DOCUMENTS';
export const GET_SITE_LOGS = 'GET_SITE_LOGS';
export const GET_SITE_FORMS = 'GET_SITE_FORMS';
export const GET_RECENT_PICTURES = 'GET_RECENT_PICTURES';
export const GET_SUBSITES = 'GET_SUBSITES';
export const SHOW_DOT_LOADER = 'SHOW_DOT_LOADER';
export const SHOW_DASHBOARD_LOADERS = 'SHOW_DASHBOARD_LOADERS';
export const UPDATE_SITE_LOGO = 'UPDATE_SITE_LOGO';
// export const SITE_DASHBOARD_ERR = "SITE_DASHBOARD_ERR";
// export const SITE_METAS_ERR = "SITE_METAS_ERR";
// export const SITE_SUBMISSIONS_ERR = "SITE_SUBMISSIONS_ERR";
// export const SITE_DOCUMENTS_ERR = "SITE_DOCUMENTS_ERR";
// export const SITE_PICTURES_ERR = "SITE_PICTURES_ERR";
// export const SITE_LOGS_ERR = "SITE_LOGS_ERR";

// Project Dashbaord Actions
export const GET_PROJECT_DASHBOARD = 'GET_PROJECT_DASHBOARD';
export const GET_PROJECT_METAS = 'GET_PROJECT_METAS';
export const GET_PROJECT_SUBMISSIONS = 'GET_PROJECT_SUBMISSIONS';
export const GET_PROJECT_DOCUMENTS = 'GET_PROJECT_DOCUMENTS';
export const GET_PROJECT_LOGS = 'GET_PROJECT_LOGS';
export const GET_REGION_DATA = 'GET_REGION_DATA';
export const GET_PROGRESS_TABLE_DATA = 'GET_PROGRESS_TABLE_DATA';
export const GET_SURVEY_FORM = 'GET_SURVEY_FORM';
// // export const GET_SUBSITES = "GET_SUBSITES";
// export const SHOW_DOT_LOADER = "SHOW_DOT_LOADER";
export const SHOW_PROJECT_DASHBOARD_LOADERS =
  'SHOW_PROJECT_DASHBOARD_LOADERS';
export const UPDATE_PROJECT_LOGO = 'UPDATE_PROJECT_LOGO';

// Team Dashboard Actions
export const GET_TEAM_DASHBOARD = 'GET_TEAM_DASHBOARD';
export const SHOW_TEAM_DASHBOARD_LOADERS =
  'SHOW_TEAM_DASHBOARD_LOADERS';
export const POST_PACKAGE_SUBSCRIBE = 'POST_PACKAGE_SUBSCRIBE';

// User Document Actions
export const GET_USER_DOCUMENT = 'GET_USER_DOCUMENT';

// Site Team Users
export const GET_TEAM_USERS = 'GET_TEAM_USERS';

// Project List
export const GET_PROJECT_LIST = 'GET_PROJECT_LIST';

// Project User List
export const GET_PROJECT_USER = 'GET_PROJECT_USER';

// Project Log
export const GET_LOGS = 'GET_LOGS';

// Teams
export const GET_TEAMS = 'GET_TEAMS';
export const GET_TRANSLATION = 'GET_TRANSLATION';

// ProjectViewData
export const GET_PROJECT_VIEW = 'GET_PROJECT_VIEW';

// ManageForms
export const FORM_LOADER = 'FORM_LOADER';
export const GET_MY_FORM_LIST = 'GET_MY_FORM_LIST';
export const GET_PROJECT_FORM_LIST = 'GET_PROJECT_FORM_LIST';
export const GET_SHARED_FORM_LIST = 'GET_SHARED_FORM_LIST';
export const GET_REGIONS_AND_TYPES = 'GET_REGIONS_AND_TYPES';
// SiteviewData
export const GET_SITE_VIEW = 'GET_SITE_VIEW';

// Translation
// export const GET_TRANSLATION = "GET_TRANSLATION";

// Reports
export const GET_METRICS_DATA = 'GET_METRICS_DATA';
export const GET_FORM = 'GET_FORM';
export const GET_TO_FILTER_DATA = 'GET_TO_FILTER_DATA';
export const GET_FORM_QUESTIONS = 'GET_FORM_QUESTIONS';
export const REPORT_LOADER = 'REPORT_LOADER';
export const GET_REPORTS_LIST = 'GET_REPORTS_LIST';
export const APPLY_ACTION_TO_REPORT = 'APPLY_ACTION_TO_REPORT';
export const GET_REPORT_DATA = 'GET_REPORT_DATA';
export const GET_PROJECT_DETAILS = 'GET_PROJECT_DETAILS';
export const GET_CUSTOM_REPORT_TABLE_DATA =
  'GET_CUSTOM_REPORT_TABLE_DATA';

// superAdminDashboard
export const GET_SUPERADMIN_DASHBOARD = 'GET_SUPERADMIN_DASHBOARD';
export const SHOW_SUPERADMIN_DASHBOARD_LOADER =
  'SHOW_SUPERADMIN_DASHBOARD_LOADER';
export const GET_ADMIN_REPORTS_LIST = 'GET_ADMIN_REPORTS_LIST';
export const GET_ORG_EXPORT_LIST = 'GET_ORG_EXPORT_LIST';
export const CREATE_ORG_EXPORT = 'CREATE_ORG_EXPORT';
export const DELETE_ORG_EXPORT = 'DELETE_ORG_EXPORT';

// MAPFILTER
export const GET_PRIMARY_MARKER_GEOJSON = 'GET_PRIMARY_MAKER_GEOJSON';
export const GET_SECONDARY_MARKER_GEOJSON =
  'GET_SECONDARY_MAKER_GEOJSON';
export const GET_PROJECTS_LIST = 'GET_PROJECTS_LIST';
export const GET_PROJECTS_REGION_TYPES = 'GET_PROJECTS_REGION_TYPES';
export const FILTER_PRIMARYGEOJSON = 'FILTER_PRIMARYGEOJSON';
export const SEARCH_PRIMARYGEOJSON = 'SEARCH_PRIMARYGEOJSON';
export const REFRESH_GEOJSONDATA = 'REFRESH_GEOJSONDATA';
export const GET_ADMIN_PROGRESS_TABLE_LIST =
  'GET_ADMIN_PROGRESS_TABLE_LIST';
export const GET_GEOLAYERS_LIST = 'GET_GEOLAYERS_LIST';
export const GET_FORM_LIST_DATA = 'GET_FORM_LIST_DATA';
export const GET_QUESTION_LIST_FORMDATA =
  'GET_QUESTION_LIST_FORMDATA';

// Excel Exports
export const GET_EXPORT_LIST = 'GET_EXPORT_LIST';
export const CREATE_EXPORT = 'CREATE_EXPORT';
export const DELETE_EXPORT = 'DELETE_EXPORT';
export const DOWNLOAD_EXPORT = 'DOWNLOAD_EXPORT';

// templates
export const GET_FORM_TYPES = 'GET_FORM_TYPES';
export const GET_REPORT_LIST = 'GET_REPORT_LIST';
export const GET_REGIONS = 'GET_REGIONS';
export const GET_TYPES = 'GET_TYPES';
export const GET_PROJECT_BREADCRUMB = 'GET_PROJECT_BREADCRUMB';
export const GENERATE_DATA_EXPORT = 'GENERATE_DATA_EXPORT';
export const EXPORT_EXCEL_REPORT = 'EXPORT_EXCEL_REPORT';
