...
export default class ChangePassword extends React.Component {
  ...
  handleChange = e => {
    ...
    this.setState(state => ({
      ...
      this.setState(() => {
        if (errors[name] && name === 'oldPwd') return { errors: { oldPwd: '' } }
        if (errors[name] && name === 'newPwd') return { errors: { newPwd: '' } }
        if (errors[name] && name === 'confirmPwd') return { errors: { confirmPwd: '' } }
      })
    });
  };

  handlePasswordValidation = () => {
    const testRegex = new RegExp("^(((?=.*[a-z])(?=.*[A-Z]))|((?=.*[a-z])(?=.*[0-9]))|((?=.*[A-Z])(?=.*[0-9])))(?=.{6,})");
    const errors = {}
    // this.setState(state => {
    ...
    if (oldPwd && newPwd && oldPwd === newPwd) {
      return { errors: { newPwd: 'New password cannot be same as Old password.' } }
    }
    ...
    if (newPwd && !testRegex.test(newPwd)) {
      return {
        errors: {
          newPwd: 'The new password must contains at least  one lowercase ' +
  ...
  handleSubmitForm = () => {
    const { form: { oldPwd, newPwd } } = this.state
  ...
  render() {
    const { form: { oldPwd, newPwd, confirmPwd }, errors } = this.state
            ...
            <form className="edit-form" onSubmit={e => {
              ...
              <div className="col-xl-4 col-md-6">
                ...
                {errors && errors.oldPwd &&
                  <small style={{ color: 'red' }}>*{errors.oldPwd}</small>
                }
              </div>
              <div className="col-xl-4 col-md-6">
                <InputElement
                ...
                {errors && errors.newPwd &&
                  <small style={{ color: 'red' }}>*{errors.newPwd}</small>
                }
              ...
              <div className="col-xl-4 col-md-6">
                ...
                {errors && errors.confirmPwd &&
                  <small style={{ color: 'red' }}>*{errors.confirmPwd}</small>
                }
